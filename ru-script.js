...
function whenButtonClicked() {
    ...
    function getComputerSelection() {
        ...
        let signArray = [ "камень" , "бумага" , "ножницы" ];
    ...
    yourSignDiv.innerHTML = `<img src="assets/${playerSelection}.png" 
        alt="Твой знак" class = "wrapper__img-${playerSelection}">`;
    yourSignDiv.classList.add('your-choice');

    compSignDiv.innerHTML = `<img src="assets/${computerSelection}.png" 
        alt="Знак Компьютер" class = "wrapper__img-${computerSelection}">`;
    compSignDiv.classList.add('comp-choice')

    function playRound() {

        if (
            (playerSelection == "камень" && computerSelection == "камень") ||
            (playerSelection == "бумага" && computerSelection == "бумага") ||
            (playerSelection == "ножницы" && computerSelection == "ножницы")
        ) {
            roundOutcomeText.textContent = 
                `${playerSelection.charAt(0).toUpperCase()}${playerSelection.slice(1)}
                = ${computerSelection}, ничья!`;
        } else if (
            ...
            roundOutcomeText.textContent = 
                `${playerSelection.charAt(0).toUpperCase()}${playerSelection.slice(1)}
                > ${computerSelection}, ты победил в этом раунде!`;
            yourSignDiv.classList.add('won');
            compSignDiv.classList.add('lost');
            playerCounter++;
            yourScore.textContent = `Ты: ${playerCounter}`;
        } else {
            roundOutcomeText.textContent = 
                `${computerSelection.charAt(0).toUpperCase()}${computerSelection.slice(1)} 
                > ${playerSelection}, ты проиграл в этом раунде!`;
            yourSignDiv.classList.add('lost');
            compSignDiv.classList.add('won');
            computerCounter++;
            compScore.textContent = `Компьютер: ${computerCounter}`;
        }
    ...
    function endTheGame() {
        ...
        if (playerCounter === 3) {
            ...
            `<span class="outcome-highlight">Победа!</span><br> 
            ${playerCounter} : ${computerCounter}`;
        } else {
            gameOutcomeText.innerHTML = 
            `<span class="outcome-highlight">Поражение!</span><br>
            ${playerCounter} : ${computerCounter}`;
        }
    ...
    function resetToDefaultState() {
        ...
        roundOutcomeText.textContent = 'Выбери знак, игра до 3х побед!';
        yourSignDiv.innerHTML = '?';
        compSignDiv.innerHTML = '?';
        yourScore.textContent = 'Ты: 0';
        compScore.textContent = 'Компьютер: 0';
        playerCounter = 0;
        computerCounter = 0;
